...
export function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // Mocking auth check
  React.useEffect(() => {
    // In a real app, verify isAdmin from Firestore
    // Here we just check if they are logged in. For mock, we'll allow access if user exists
    if (!user) {
      setLocation("/admin");
    }
  }, [user, setLocation]);
    ...
    <div className="min-h-screen flex bg-muted/20 font-sans">
      ...
      <aside className="w-64 bg-[#111111] text-white flex flex-col hidden lg:flex shrink-0">
        ...
        <nav className="flex-1 py-6 px-4 space-y-2 overflow-y-auto">
          {menu.map(item => {
            ...
            // A simple active check (would need exact logic or wouter's useRoute)
            const isActive = window.location.pathname.startsWith(item.path);
            return (
              <Link 
                key={item.id} 
                href={item.path}
                className={`flex items-center gap-3 px-4 py-3 rounded-md transition-colors ${isActive ? 'bg-secondary text-black font-medium' : 'text-white/70 hover:bg-white/10 hover:text-white'}`}
              >
                <Icon size={18} />
                {item.label}
              </Link>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <header className="bg-white border-b border-border h-16 flex items-center justify-between px-6 lg:hidden">
          ...
          <button onClick={handleLogout} className="text-muted-foreground"><LogOut size={20} /></button>
        </header>
